...
public class OlusturduklarimFormu extends JFrame {
    private String kullaniciAdi;
    private Properties config;
    private JTable table;
    private DefaultTableModel model;

    public OlusturduklarimFormu(String kullaniciAdi, Properties config) {
        this.kullaniciAdi = kullaniciAdi;
        this.config = config;

        setTitle("Oluşturduğum Projeler");
        setSize(800, 500);
        setLocationRelativeTo(null);
        setLayout(new BorderLayout());

        // Tablo modeli
        String[] kolonlar = {"Proje Adı", "Açıklama", "Başlangıç Tarihi", "Bitiş Tarihi", "Görevli Sayısı"};
    ...
    private void projeleriYukle() {
        ...
        if ("mongodb".equalsIgnoreCase(dbMode)) {
            try (MongoClient mongoClient = MongoClients.create("mongodb://127.0.0.1:27017")) {
                ...
                for (Document doc : collection.find(new Document("created_by", kullaniciAdi))) {
                    model.addRow(new Object[]{
                        doc.getString("name"),
                        doc.getString("description"),
                        doc.getString("start_date"),
                        doc.getString("end_date"),
                        doc.getList("responsibles", Document.class).size()
                    });
                }
            } catch (Exception e) {
            ...
            try (java.sql.Connection conn = java.sql.DriverManager.getConnection(
                ...
                String sql = "SELECT p.name, p.description, p.start_date, p.end_date, " +
    ...
    private void projeSil() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if ("mongodb".equalsIgnoreCase(dbMode)) {
                ...
                try (java.sql.Connection conn = java.sql.DriverManager.getConnection(
                        "jdbc:mysql://localhost:3306/proje_yonetim", "root", "")) {
                    String sql = "DELETE FROM projects WHERE name = ? AND created_by = ?";
                    java.sql.PreparedStatement stmt = conn.prepareStatement(sql);
